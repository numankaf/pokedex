...
const EditPokemonPage = ({params}) => {
    ...
    const getPokemon = async () => {
        await pokemonService.getById(id).then((data) => {
            setPokemon(data)
            let pokTypes = [];
            for (const t of data.types) {
                pokTypes.push({name: t});
            }
            setInput({
                name: data.name,
    ...
    const onSubmit = () => {
        ...
        pokemonService.updatePokemon(id,pokCreateData).then((res) => {
            setLoading(true)
            toast.current.show({
                severity: 'success',
                summary: 'Success',
                detail: 'You have updated the pokemon. Redirecting to back...',
                life: 3000
            });
            setTimeout(() => {
                router.back()
    ...
    return (
        <div className={"card"}>
            <Toast ref={toast}/>
            <h1 className={"text-center"}> Edit Pokemon</h1>
                    ...
                    <div className="flex flex-column  ">
                        <div className="flex flex-column align-items-center justify-content-center">
                            ...
                            <div>
                                ...
                                />
                                <div className={"shadow-2 w-15rem h-15rem img-input"}
                                ...
                                <div className="py-3 flex justify-content-between flex-wrap">
                                    ...
                                    <span style={{"width": "24%"}}>
                                        <label htmlFor="types"
                                         ...
                                         <MultiSelect value={input.types || ''} options={types}
                                                      onChange={(e) => setInput({...input, types: e.target.value})}
                                                      optionLabel="name"
                                                      placeholder="Select Types" itemTemplate={typeOptionTemplate}
